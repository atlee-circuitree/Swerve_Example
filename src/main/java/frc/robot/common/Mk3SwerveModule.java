...
public class Mk3SwerveModule extends SwerveModule {
    ...
    private Notifier canUpdateNotifier = new Notifier(() -> {
        ...
        if (faults.UnderVoltage || faults.ResetDuringEn) {
            ...
            steeringMotor.clearStickyFaults(CAN_TIMEOUT_MS);
    ...
    public Mk3SwerveModule(Vector2 modulePosition, double angleOffset, double angleGearRatio, double driveGearRatio,
        ...
        steeringMotor.configSelectedFeedbackSensor(TalonFXFeedbackDevice.IntegratedSensor, TALONFX_PID_LOOP_NUMBER, CAN_TIMEOUT_MS);
        ...
        steeringMotor.configFeedbackNotContinuous(true, CAN_TIMEOUT_MS);
        // Configure PID values
        steeringMotor.config_kP(TALONFX_PID_LOOP_NUMBER, 0.3, CAN_TIMEOUT_MS);
        steeringMotor.config_kI(TALONFX_PID_LOOP_NUMBER, 0.0, CAN_TIMEOUT_MS);
        steeringMotor.config_kD(TALONFX_PID_LOOP_NUMBER, 0.0, CAN_TIMEOUT_MS);
        // Limit steering module speed
        steeringMotor.configPeakOutputForward(MAX_STEERING_SPEED, CAN_TIMEOUT_MS);
        steeringMotor.configPeakOutputReverse(-MAX_STEERING_SPEED, CAN_TIMEOUT_MS);
